<template>
  <div class="container mx-auto nav pa-4 rounded-lg my-10">
    <h1 class="text-3xl font-bold my-4 text-center">{{ $t('Top 10 stocks in terms of percentage change') }}</h1>
    ...
    <v-data-table v-if="!loading" :headers="headers" :items="topStocks" class="elevation-1 nav" hide-default-footer
...
onMounted(async () => {
  try {
    ...
    // Take top 5 stocks
    topStocks.value = stocks.slice(0, 10);
