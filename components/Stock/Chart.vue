...
async function fetchData() {
  try {
    ...
    const historicalResponse = await fetch(
      `https://financialmodelingprep.com/api/v3/historical-price-full/${stockSymbol.value}?apikey=yJ2JzqBMsGlz3rV7rkogCtrEc7eY6QDh`
    );
    const historicalJson = await historicalResponse.json();
    data.value = historicalJson.historical.slice(0, 50);

    // جلب اسم السهم
    const nameResponse = await fetch(
      `https://financialmodelingprep.com/api/v3/search?query=${stockSymbol.value}&apikey=yJ2JzqBMsGlz3rV7rkogCtrEc7eY6QDh`
    );
    const nameJson = await nameResponse.json();
    if (nameJson.length > 0) {
      stockName.value = nameJson[0].name;
    }
  ...
  onMounted(() => {
    if (data.value) {
      ...
      Highcharts.stockChart('container', {
        ...
        title: {
          text: `${stockName.value} Price and Volume`,
          style: {
            color: '#2caffe'
          }
        },
