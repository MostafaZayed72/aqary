    ...
    <v-card :title="$t('Filter stocks by trending')" flat class="nav rounded-lg text-center mx-auto sm:w-100 md:w-[90%]">
      ...
      <v-chip-group v-model="selectedTrending" class="mt-4 text-center">
        <div class="flex justify-center mx-auto text-center gap-1 md:grid-cols-4 lg:grid-cols-6">
          <v-chip :key="'Uptrend'" :value="'uptrend'" class="ma-1 hover:bg-teal-400 delayed text-center flex justify-center items-center rounded-lg w-[150px]"
            color="primary" label="true">
            {{ $t('Uptrend') }}
          </v-chip>
          <v-chip :key="'Downtrend'" :value="'downtrend'" class="ma-1 hover:bg-teal-400 delayed text-center flex justify-center items-center rounded-lg w-[150px]"
            color="primary" label="true">
            {{ $t('Downtrend') }}
          </v-chip>
        </div>
  ...
  const fetchStocks = async () => {
    ...
    try {
      const [stocksResponse, trendsResponse] = await Promise.all([
        fetch('https://development.somee.com/api/StockMarket/GetAllSymbolData?apikey=pYR3gnW9oyf6juDsf5rtdP7hs2d8wuHg'),
        fetch('https://development.somee.com/api/StockMarket/GetAllSymbolsTechnicalAnalysis?apikey=pYR3gnW9oyf6juDsf5rtdP7hs2d8wuHg')
      ]);
